...
class GraphPartition:
    ...
    def community_linkage(self, communities, full=True):
        ...
        for i in range(num_batches):
            ...
            x_batch = x_com[start_idx:end_idx]
            diff = x_batch.unsqueeze(1) - x_com.unsqueeze(0)
            batch_linkage.append(torch.linalg.norm(diff, dim=2))
        linkage = torch.cat(batch_linkage, dim=0)
        # are_equal = torch.equal(linkage, linkage_diff)
        # print(are_equal)
